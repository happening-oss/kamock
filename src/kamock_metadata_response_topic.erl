...
-export([
    make_metadata_response_topic/2,
    make_metadata_response_topic/3
]).
-export([
    partitions/1,
    partitions/2
...
-define(DEFAULT_PARTITION_COUNT, 4).

make_metadata_response_topic(Topic, Env) ->
    make_metadata_response_topic(Topic, ?DEFAULT_PARTITION_COUNT, Env).
...
partitions(PartitionCount) when is_integer(PartitionCount) ->
    fun(Topic, Env) ->
        make_metadata_response_topic(Topic, PartitionCount, Env)
    end;
partitions(PartitionCounts) when is_map(PartitionCounts) ->
    partitions(PartitionCounts, ?DEFAULT_PARTITION_COUNT).

partitions(PartitionCounts, DefaultPartitionCount) ->
    fun(Topic = #{name := TopicName}, Env) ->
        PartitionCount = maps:get(TopicName, PartitionCounts, DefaultPartitionCount),
        make_metadata_response_topic(Topic, PartitionCount, Env)
    end.
